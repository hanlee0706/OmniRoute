...
# Railway volumes are mounted as root. When Railway starts the image with
# RAILWAY_RUN_UID=0, repair the default data-volume ownership and immediately
# re-exec this entrypoint as the baked-in non-root node user. Restrict the
# automatic chown to the fixed Docker data path so a malformed DATA_DIR cannot
# make a root startup recursively change ownership elsewhere in the image.
if [ "$(id -u)" -eq 0 ]; then
    if [ "$DATA_PATH" = "/app/data" ] && [ -d /app/data ]; then
